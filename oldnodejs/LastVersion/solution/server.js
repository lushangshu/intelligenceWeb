...
var app = protocol.createServer(function (req, res) {
    ...
    if ((req.method == 'POST') && (pathname == '/postFile.html')){
        ...
        req.on('end', function () {
            ...
            client.get('search/tweets', { q:topic,c:coordinates,count:100},function (err, data, response) {           
    ...
    var stest
    var date=new Date()
    //var query=connection.query('SELECT * FROM usersInformation');
    client.get('search/tweets', { q:'swarmapp.com/c/',from:result.user_id,since:date.setDate(date.getDates-result.days)},function (err, data, response) {
        for(var indx in data.statuses) {
            var tweet = data.statuses[indx];
            var naem=tweet.user.screen_name;
            //var insert= 'INSERT INTO usersInformation(username,userid,address,description) VALUES(" '+data.statuses[0].user.name+'"," '+data.statuses[0].user.id+'"," '+data.statuses[0].user.location+'"," '+data.statuses[0].user.description+'")';
            ...
            for (var indx1 in tweet.entities.urls){
                  ...
                  request(options,function (error, response, body) {
                      ...
                      if(!error && response.statusCode == 200) {
                              ...
                              res.end(JSON.stringify(jsonObj.response.checkin.venue.name));
                             ...
                             throw err;
                              ...
                              querylocation.on('fields', function(fields) {
                              console.log(fields);
                             });  
                            querylocation.on('result', function(row) {
                                          console.log(row);
                             });
                        }
                       else 
            ...
            request(options,function(error,response,body){
                if(!error && response.statusCode == 200){
                    ...
                    for(var indx=0; indx<jsonRet.response.venues.length;indx++){
                        ...
                        {   
                            //console.log(jsonRet.response.venues.length);
                            //console.log(imgUrl+' ***'); 
                            res.writeHead(200, { "Content-Type": "application/text", 'Access-Control-Allow-Origin': '*'});
                            res.end(JSON.stringify(nameArray+'**'+imgUrl));
    ...
    else  if ((req.method == 'POST') && (pathname == '/postFile4.html')){
        var pathname = url.parse(req.url).pathname;
        var body = '';
        var dataFin = {ok: 'ok'};
        var dataFin = {ok: 'ok'};
...
//                    console.log(sortKW);
                ...
                res.writeHead(200, { "Content-Type": "application/json", 'Access-Control-Allow-Origin': '*'});
                res.end(JSON.stringify(finalTotal));
            }
        }
    }     
}
